...
package featureExtractor.vision;

import ab.demo.other.ActionRobot;
import featureExtractor.vision.real.ConnectedComponent;
import featureExtractor.vision.real.ImageSegmenter;
import featureExtractor.vision.real.shape.Body;
import featureExtractor.vision.real.shape.Circle;
import featureExtractor.vision.real.shape.Rect;
...
public class VisionRealShape 
{
    ...
    // find all objects in the scene beside slingshot, birds, pigs
    ...
    // find hills, taken from old version because of mis-detection of hills in version 1.31
    public List<ABObject> myfindHills()
    {
    	ArrayList<ABObject> _gameObjects = new ArrayList<ABObject>();
        int xMin = 0;
        if (_sling != null)
            xMin = _sling.x + 100;
                ...
                c.getType() == ImageSegmenter.HILLS)
            {
               ...
               /* if(c.getType() == ImageSegmenter.PIG)
                ...
                if ( b instanceof Circle && b.type == ABType.Wood)
    			{
    				...
    				ABObject newBlock = new Rect(b.getCenterX(), b.getCenterY(), b.getBounds().width, b.getBounds().height, 0, ABType.Unknown, b.area);
        ...
        for(int i=0; i<_gameObjects.size(); i++){
        	if(!_gameObjects.get(i).type.toString().equals("Hill")){
