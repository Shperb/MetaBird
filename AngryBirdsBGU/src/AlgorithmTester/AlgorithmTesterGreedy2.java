...
public class AlgorithmTesterGreedy2 extends AlgorithmTester {
	...
	public AlgorithmTesterGreedy2(Problem pProblem) throws Exception {
		super(pProblem);
	...
	protected String getName() {
		return "Greedy2(score per second)";
	}
	...
	private int getHighestAdditionalUtility(Game pGame, String[] refChoice) {
		...
		mScoresDistribution.forEach((agent, v) -> {
			v.forEach((level, distribution) -> {
				double exp = distribution.getExpectation(scores.get(level)) / mTimeDistribution.get(agent).get(level).getExpectation();
				if (exp > retVal[0]) {
					retVal[0] = exp;
					refChoice[0] = agent;
					refChoice[1] = level;
