...
//			System.out.println(mProblem + "\t" + getName() + "\t" + gameScore + "\t" + getAdditionalData());
    ...
    public double[] getAverageScoreTimeRateOnce(int pRepetitionsCount) throws Exception {
        long[] additionalTime = new long[1];
        double avarageScore = 0;
        double avarageTime = 0;
        for (int i=0; i<pRepetitionsCount; i++) {
            double[] result = testOnce(additionalTime);
            double gameScore = result[0];
            double gameTime = result[1];
            avarageScore = (1.0/(i+1))*(gameScore + i * avarageScore );
            avarageTime = (1.0/(i+1))*(gameTime + i * avarageTime );
        }
...
//		System.out.println("selected levels: " + String.join(",", game.levelNames));
    ...
    protected double[] testOnce(long[] additionalTime) throws Exception {
