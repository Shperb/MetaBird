...
public class ClientMessageEncoder {
	...
	public static byte[] configure(byte[] id) {
		...
		message = mergeArray(new byte[] { ClientMessageTable.getValue(ClientMessageTable.configure) }, id);
	...
	public static byte[] cshoot(byte[] fx, byte[] fy, byte[] dx, byte[] dy, byte[] t1, byte[] t2) {
		...
		message = mergeArray(new byte[] { ClientMessageTable.getValue(ClientMessageTable.cshoot) }, fx, fy, dx, dy, t1,
				t2);
		return message;
	...
	public static byte[] pshoot(byte[] fx, byte[] fy, byte[] dx, byte[] dy, byte[] t1, byte[] t2) {
		byte[] message = new byte[1 + fx.length + fy.length + dx.length + dy.length + t1.length + t2.length];
		message = mergeArray(new byte[] { ClientMessageTable.getValue(ClientMessageTable.pshoot) }, fx, fy, dx, dy, t1,
				t2);
		return message;
	}
