...
public class ClientActionRobotJava extends ClientActionRobot {
	...
	public GameState checkState() {
		...
		System.out.println("in check state result is " + result);
	...
	public byte[] shoot(int fx, int fy, int dx, int dy, int t1, int t2, boolean polar, TrajectoryPlannerHeartyTian tp,
			Rectangle sling, ABType birdOnSling, List<ABObject> blocks, List<ABObject> birds, int nOfShots) {
		// return super.shootFast(intToByteArray(fx), intToByteArray(fy),
		// intToByteArray(dx), intToByteArray(dy), intToByteArray(t1),
		// intToByteArray(t2), polar, tp, sling, birdOnSling, blocks, birds,
		// nOfShots);
		return super.shootSafe(intToByteArray(fx), intToByteArray(fy), intToByteArray(dx), intToByteArray(dy),
				intToByteArray(t1), intToByteArray(t2), polar, tp, sling, birdOnSling, blocks, birds, nOfShots);
	}
	...
	public byte[] cshootSequence(int[]... shots) {
		...
		for (int[] shot : shots) {
			byteShots[shotCount] = ClientMessageEncoder.mergeArray(intToByteArray(shot[0])/* fx */,
					intToByteArray(shot[1])/* fy */, intToByteArray(shot[2])/* dx */, intToByteArray(shot[3])/* dy */,
					intToByteArray(shot[4])/* t1 */, intToByteArray(shot[5])/* t2 */);
			shotCount++;
		}
