...
public class VisionRealShape {
	...
	public Rectangle findSling() {
		...
		for (ConnectedComponent c : _components) {
			...
			if (c.getType() == ImageSegmenter.SLING && top < minY
			// case when found tnt
		...
		for (ConnectedComponent c : _components) {
			if (c.getType() > ImageSegmenter.SLING && c.getType() < ImageSegmenter.PIG) {
				...
				for (ABObject pig : _pigs) {
					if (pig.x - 2 < bound[2] && bound[0] < pig.x + pig.width + 2 && pig.y - 2 < bound[1]
				...
				if (// bound[0] > xMax
				(bound[0] + bound[2]) / 2 > xMax && !isPartOfThePig)
				// && bound[3] < _ground + 2)
				{
	 ...
	 * 
	 * @param canvas fill - if true, internal points of the components will be
	 * drawn
	 */
	...
	public void drawObjectsWithID(BufferedImage canvas, boolean fill) {
		...
		for (Body b : _drawShape)
			if (b != null) {
				...
				if (b.id != unassigned)
					g.drawString(b.id + "", (int) b.centerX - 5, (int) b.centerY + 5);// 10:
																						// font
																						// size
			}
