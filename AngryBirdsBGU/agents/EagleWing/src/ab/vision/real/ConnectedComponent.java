...
public class ConnectedComponent {
	...
	/*
	 * Trace the contour using Moore-Neighbour tracing and partition contour
	 * points into line segments
	 * 
	 * @return list of line segments which form a cycle around outer border of
	...
	private Body findShape(List<Point> corners) {
		...
		for (int i = 0; i < RESOLUTION; i++) {
			...
			// rotate each point about the origin and record min/max x,y
			// coordinates
			for (Point p : corners) {
	 ...
	 * 
	 * @param canvas drawEdge - if border should be drawn drawCorner - if the
	 * corner points should be drawn
	 */
	...
	private final static int xClock[][] = { { 0, -1, -1 }, { 0, 0, 0 }, { 1, 1, 0 } };
	private final static int yClock[][] = { { 1, 0, 0 }, { 1, 0, -1 }, { 0, 0, -1 } };
	 ...
	 * 
	 * @return point connected to p which is in the anticlockwise direction from
	 * p to prev
	 */
