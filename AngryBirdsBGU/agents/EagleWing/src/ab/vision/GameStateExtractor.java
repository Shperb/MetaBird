...
public class GameStateExtractor {
	...
	public GameStateExtractor() {
		try {
			...
			_gamecontinue = ImageIO.read(getClass().getResource("resources/continue.png"));
			_eagle = ImageIO.read(getClass().getResource("resources/eagle.png"));
			_tutorialok = ImageIO.read(getClass().getResource("resources/tutorialok.png"));
	...
	public GameState getGameState(BufferedImage screenshot) {
		...
		numBytes = 3 * wnd.getWidth() * wnd.getHeight();
		if (VisionUtils.imageDifference(wnd, _gamecontinue) < numBytes * avgColourThreshold) {
			return GameState.CONTINUE;
		}
		wnd = screenshot.getSubimage(151, 330, 41, 40);
		numBytes = 3 * wnd.getWidth() * wnd.getHeight();
		if (VisionUtils.imageDifference(wnd, _eagle) < numBytes * avgColourThreshold) {
			return GameState.EAGLE;
		}
		wnd = screenshot.getSubimage(497, 318, 28, 25);
		numBytes = 3 * wnd.getWidth() * wnd.getHeight();
		if (VisionUtils.imageDifference(wnd, _tutorialok) < numBytes * avgColourThreshold) {
	...
	public int getScoreInGame(BufferedImage screenshot) {
		...
		for (int i = 0; i < letters.length; i++) {
			...
			BufferedImage letterImage = scoreImage.getSubimage(letters[i].x, letters[i].y, letters[i].width,
					letters[i].height);
	...
	public int getScoreEndGame(BufferedImage screenshot) {
		...
		/*
		 * if (score != prevScore) { saved = false; repeatCount = 0; prevScore =
		 * score; } else if (score != 0 && !saved) { repeatCount++; if
		 * (repeatCount > 0) { saved = true; try { File outputfile = new
		 * File("scoreImage/" + score + ".png"); ImageIO.write(saveImage, "png",
