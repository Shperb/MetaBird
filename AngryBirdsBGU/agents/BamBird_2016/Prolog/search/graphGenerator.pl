...
createEdge(Root, Target, BadOrientation,NewEdge) :-
	findall(
		...
		(
			...
			(	(hasEdgeToWith(Root,Target, Cost)) -> true;
				asserta(hasEdgeToWith(Root, Target, Cost))),
			Edge = hasEdgeToWith(Root, Target, Cost); false
		),
		Edges
	), pSysoSeGG('Edges', Edges), nth0(0, Edges, NewEdge).
...
pSysoSeGG(_Test, _Value) :- %syso(_Text, _Value),
	true.